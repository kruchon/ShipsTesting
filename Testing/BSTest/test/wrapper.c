...
extern void xOutEnv( xSignalNode *SignalOut )
{
	...
	if(((*SignalOut)->NameNode)==booms) {
		battleshipstest_Recv_booms(((yPDP_booms)(*SignalOut))->Param1,TAT_I_Strategy_ID,TAT_RECEIVER);
		xReleaseSignal(SignalOut);
		return;
	}
	if(((*SignalOut)->NameNode)==found) {
		battleshipstest_Recv_found(((yPDP_found)(*SignalOut))->Param1,((yPDP_found)(*SignalOut))->Param2,TAT_I_Strategy_ID,TAT_RECEIVER);
		xReleaseSignal(SignalOut);
		return;
	}
...
TAT_SIG_booms model_sigbooms = { 0 };
TAT_INSTANCE_ID instances_evbooms[] = {0, 0};
TAT_EVENT_INSTANCE model_evbooms = { TAT_RECV_EV, TAT_S_booms_ID, &model_sigbooms, {{0,0}, {0,0}, ""},2,
 instances_evbooms, NULL, 0, NULL, TAT_VERDICT_PASS, -1, NULL, NULL};

TAT_SIG_found model_sigfound = { 0, 0 };
TAT_INSTANCE_ID instances_evfound[] = {0, 0};
TAT_EVENT_INSTANCE model_evfound = { TAT_RECV_EV, TAT_S_found_ID, &model_sigfound, {{0,0}, {0,0}, ""},2,
 instances_evfound, NULL, 0, NULL, TAT_VERDICT_PASS, -1, NULL, NULL};
...
/************************************************************************************
*
* Function    : battleshipstest_Recv_booms
*
* Description : This function transfer signals from testing system to test
*
* Return      : 1
...
*
*************************************************************************************/
int battleshipstest_Recv_booms(int Param1, int from, int to)
{
	model_sigbooms.Param1 = Param1;
	instances_evbooms[0] = from;
	instances_evbooms[1] = to;
	TAT_ReceiveMessage(TI_ID, &model_evbooms);
	return 1;
}

/************************************************************************************
*
* Function    : battleshipstest_Recv_found
*
* Description : This function transfer signals from testing system to test
*
* Return      : 1
...
*
*************************************************************************************/
int battleshipstest_Recv_found(int Param1, int Param2, int from, int to)
{
	model_sigfound.Param1 = Param1;
	model_sigfound.Param2 = Param2;
	instances_evfound[0] = from;
	instances_evfound[1] = to;
	TAT_ReceiveMessage(TI_ID, &model_evfound);
	return 1;
}
